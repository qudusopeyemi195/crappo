...
const Button = ({ children, classProps }) => {
  return (
    <button className={`flex ${classProps} px-3 py-2 items-center active:-translate-y-1 cursor-pointer hover:-translate-y-2 duration-300 text-white font-[600] bg-[#3b82f6] rounded-2xl rounded-tl-none rounded-br-none`}>
      {children}
    </button>
  );
};
...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 z-[100] w-full h-20 md:px-10 px-4 ${navBg 
      ? 'shadow-xl z-[100] bg-[#0D0D2B] ease-in-out duration-300' 
      : ''}`} 
     > {/**Maybe to remove top-0 */}
      <div className="flex justify-between items-center w-full h-full max-w-[1200px] m-auto"> {/**max-w-[1200px] */}
        <Link to="hero" smooth={true} offset={50} duration={500}>
          <div className="flex justify-beween items-center py-2 space-x-4 cursor-pointer">
      ...
      {/* Mobile Menu */}
      {/* Overlay */}
      <div className={toggleMenu ? 'md:hidden fixed left-0 top-0 w-full h-screen bg-lack/70' : ''}>
        {/* Side Drawwer Menu */}
        ...
        >
          <div>
            <div className="flex justify-between items-center w-full">
              <Link to="hero" smooth={true} offset={50} duration={500} onClick={() => setToggleMenu(false)}>
                <div className="flex justify-between items-center space-x-4 py-2 cursor-pointer">
                  <img src={Logo} alt="" width={40} />
                  <h1 className="text-lg">CRAPPO</h1>
                </div>
              </Link>
              <div className="p-3 rounded-full shadow-sm cursor-pointer" onClick={() => setToggleMenu(false)}>
                <AiOutlineClose />
              </div>
            </div>
